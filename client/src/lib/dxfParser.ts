...
import { Vertex, Opening, DxfView } from "./types";
...
function parseDxfEntities(content: string) {
  ...
  while (i < lines.length) {
    ...
    if (code === "0" && (value === "3DFACE" || value === "MESH" || value === "3DSOLID" || value === "BODY" || value === "REGION")) {
      while (i < lines.length) {
        if (peek() === "0") break;
        next(); next();
      }
...
export function parseDxfFile(content: string): DxfParseResult {
  ...
  const parentIdx: number[] = closedPolylines.map(() => -1);
  for (let ci = 0; ci < closedPolylines.length; ci++) {
    const centroid = polygonCentroid(closedPolylines[ci].points);
    for (let pi = 0; pi < ci; pi++) {
      if (pointInPolygon(centroid, closedPolylines[pi].points)) {
        if (parentIdx[ci] === -1 || polygonArea(closedPolylines[pi].points) < polygonArea(closedPolylines[parentIdx[ci]].points)) {
          parentIdx[ci] = pi;
        }
      }
    }
  ...
  const toVertex = (p: Point2D): Vertex => ({ id: crypto.randomUUID(), x: round3(p.x - minX), y: round3(p.y - minY) });
  const toOpening = (pts: Point2D[]): Opening => {
    const verts = pts.map(p => ({ x: round3(p.x - minX), y: round3(p.y - minY) }));
    let oMinX = Infinity, oMinY = Infinity, oMaxX = -Infinity, oMaxY = -Infinity;
    for (const v of verts) {
      oMinX = Math.min(oMinX, v.x); oMinY = Math.min(oMinY, v.y);
      oMaxX = Math.max(oMaxX, v.x); oMaxY = Math.max(oMaxY, v.y);
    }
    return { id: crypto.randomUUID(), x: oMinX, y: oMinY, width: oMaxX - oMinX, height: oMaxY - oMinY, type: "polygon", vertices: verts };
  };

  const topLevelIndices = closedPolylines.map((_, ci) => ci).filter(ci => parentIdx[ci] === -1);

  const dxfViews: DxfView[] = topLevelIndices.map((topIdx, viewIdx) => {
    const polygon = closedPolylines[topIdx].points.map(toVertex);
    const childIndices = closedPolylines.map((_, ci) => ci).filter(ci => parentIdx[ci] === topIdx);
    const openings: Opening[] = childIndices.map(ci => toOpening(closedPolylines[ci].points));

    circles.forEach(c => {
      if (pointInPolygon({ x: c.cx, y: c.cy }, closedPolylines[topIdx].points)) {
        const cx = round3(c.cx - minX);
        const cy = round3(c.cy - minY);
        const r = round3(c.r);
        openings.push({ id: crypto.randomUUID(), x: cx - r, y: cy - r, width: r * 2, height: r * 2, type: "circle" });
      }
    });

    const VIEW_NAMES = ["Face View", "Top View", "Side View", "Section View"];
    return {
      id: crypto.randomUUID(),
      name: VIEW_NAMES[viewIdx] ?? `View ${viewIdx + 1}`,
      polygon,
      openings,
      showCentroid: viewIdx === 0,
    };
  });
  ...
  for (const ol of openLines) {
    for (let k = 0; k < ol.points.length - 1; k++) {
      sketchLines.push({
        x1: round3(ol.points[k].x - minX),
        y1: round3(ol.points[k].y - minY),
        x2: round3(ol.points[k + 1].x - minX),
  ...
  const primaryView = dxfViews[0];
  const perimeter: Vertex[] = primaryView?.polygon ?? [];
  const openings: Opening[] = primaryView?.openings ?? [];

  const width = round3(maxX - minX);
  const height = round3(maxY - minY);

  return { perimeter, openings, dxfViews, nodes, sketchLines, width, height };
}
